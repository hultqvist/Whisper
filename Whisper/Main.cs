...
namespace Whisper
{
	class MainClass
	{
		public static void Main(string[] args)
		{
			...
			Sender send = new Sender("Source", storage, senderKey, recepientKey.PublicKey);
			send.Run();

			Console.WriteLine("Receiving...");
			Receiver receive = new Receiver(storage, "TargetA", recepientKey, senderKey.PublicKey);
			receive.Run();

			Console.WriteLine("Receiving 2...");
			DiskStorage myself = new DiskStorage("StorageB");
			Receiver receiveMyself = new Receiver(myself, "TargetB", recepientKey, senderKey.PublicKey);
			receiveMyself.Run();
